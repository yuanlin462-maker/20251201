console.log('Loaded sketch.js (root)');
// Debug 開關
let showDebugOnScreen = false; // 是否顯示左上角與縮圖的 debug 訊息（預設關閉）
// 多動畫設定
let animations = {
  stop: { img: null, srcCandidates: ['1/stop_1.png', 'stop/0.png'], frameWidth: 46, frameHeight: 47, totalFrames: 1 },
  go: { img: null, srcCandidates: ['1/go_1.png', 'go/all.png'], frameWidth: 154, frameHeight: 51, totalFrames: 4 },
  punch: { img: null, srcCandidates: ['1/打_1.png', '打/all.png'], frameWidth: 55, frameHeight: 60, totalFrames: 13 }
};
let currentAnim = 'stop';
let isVerticalSheet = false; // 會在檢測時更新
let currentFrame = 0;
...
function draw() {
  ...
  // 決定當前 animation: one-shot punch > any movement (horizontal/vertical) -> stop
  if (oneShotAnimName) {
    currentAnim = oneShotAnimName;
  } else if (keyRightPressed || keyLeftPressed || keyUpPressed || keyDownPressed) {
    currentAnim = 'go';
  } else {
    currentAnim = 'stop';
  }
  ...
  if (oneShotAnimName === currentAnim) {
    ...
    const elapsed = frameCount - oneShotStartFrame;
    const frameIndex = floor(elapsed / 10);
  ...
  if (showDebugOnScreen) {
    ...
    // Debug: 顯示目前的幀與裁切座標
    if (anim && anim.img) {
      const cols = floor(anim.img.width / anim.frameWidth);
      const debugSx = sx;
      const debugSy = sy;
